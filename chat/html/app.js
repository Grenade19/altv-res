...
window.addEventListener('load', function(){
  ...
  addString('<b>Pomyślnie połączono z serwerem Aelios.pl</b>');
	alt.emit('chatloaded');
});
...
function addMessage(name, text) {
	if(messagesBlock.children.length > 100)
		messagesBlock.removeChild(messagesBlock.children[0]);
	var p = document.createElement('p');
	p.innerHTML = '<div style="padding: 0.25vw; margin: 0.25vw; background-color: rgba(48, 43, 43, 0.6); border-radius: 3px;"></i>LOOC | <b>' + name + ': </b></br>' + colorify(text);
	messagesBlock.appendChild(p);
	checkOverflow();
	highlightChat();
}
